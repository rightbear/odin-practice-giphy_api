...
searchBtn.addEventListener('click', function showNewGIF(){
  ...
  if(input.value === ""){
    alert(`It seems that you don't type anything when searching. ${wrySmileEmoji}`)
  }
  else {
    ...
    fetch(`https://api.giphy.com/v1/gifs/translate?${searchParams}`, {mode: 'cors'})
    ...
    .then(function(response) {
      ...
      if(response.data.length === 0) {
        img.src = noGIF;
        setTimeout(() => {alert(`No result for the input keyword. Maybe change a new keyword? ${confuseEmoji}`)}, 1000);
      }
      // If the keyword has corresponding GIFs, show the returned result.
      else {
        img.src = response.data.images.original.url;
        setTimeout(() => {alert(`Here is the GIF you want. Enjoy it! ${goodSmileEmoji}`)}, 2000);
      }
    })
    .catch(error => {
...
catBtn.addEventListener('click', function showCatGIF(){
  fetch(`https://api.giphy.com/v1/gifs/translate?api_key=${myKey}&s=cats`, {mode: 'cors'})
    ...
    .then(function(response) {
      img.src = response.data.images.original.url;
      setTimeout(() => {alert(`You successfully summon some cute little things! ${catEmoji}`)}, 2000);
    })
    .catch(error => {
      console.log(error);
    });
